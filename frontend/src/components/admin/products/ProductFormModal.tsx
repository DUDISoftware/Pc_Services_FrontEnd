 ...
 // gọi valdate cho update 
  try {
    ...
    if (editingProduct?._id) {
      ...
      if (discountValue > 0) {
        await discountService.updateDiscount(editingProduct._id, { sale_off: discountValue , start_date: startDate, end_date: endDate});
        console.log("✅ Cập nhật giảm giá thành công:", discountValue);
      } else {
        await discountService.updateDiscount(editingProduct._id, { sale_off: 0 , start_date: new Date(), end_date: new Date()});
      ...
      if (discountValue > 0) {
        ...
        if (newestProduct?._id) {
          ...
          await discountService.createDiscount(newestProduct._id, {
